...
import { Grid, List, ListItem, Typography, Divider, Button } from "@mui/material";
...
const ShoppingCartInvoice: React.FC<IShoppingCartInvoiceProps> = () => {
  ...
  const { items } = useAppSelector(selectShoppingCart);
    ...
    items.map((price:ShoppingCartItemType) => (a += price.product.fields.priceWithDiscount ))
    console.log(a)
    ...
    >
      <List>
        ...
        <ListItem sx={{ justifyContent: "space-between" }}>
          ...
          <Typography>
            {/* {totalPrice}<span style={{ paddingRight: "10px" }}>تومان</span> */}
        ...
        <ListItem sx={{ justifyContent: "space-between" }}>
          <Typography> سود شما :</Typography>
          <Typography>
            88888888<span style={{ paddingRight: "10px" }}>تومان</span>
          </Typography>
        </ListItem>
        <ListItem
    ...
    <Grid container xs={11} sx={{ display: "flex" }}>
      ...
      <Grid container xs={10}>
        ...
        <Typography variant="overline">
          کالاهای موجود در سبد شما ثبت و رزور نشده اند. برای ثبت سفارش
          مراحل بعدی را تکمیل کنید
        </Typography>
      </Grid>
    </Grid>
    <Grid xs={11}>
      <Button variant="contained" fullWidth sx={{bgcolor:"yellow", color:"black"}}>
        پرداخت
      </Button>
    </Grid>
  </Grid>
